<script lang="ts">
    ...
    export let entry: KpiEntry | null = null;
    ...
    async function handleSubmit(event: Event) {
      ...
      try {
        const response: ApiResponse<void> = await api.updateKpiEntry(entry.id, {
          date: date,
    ...
    >
      ...
      >
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h2 class="text-xl font-bold text-gray-900">Edit KPI Entry</h2>
            <p class="mt-1 text-sm text-gray-600">Update the entry details</p>
          </div>
        ...
        <form on:submit={handleSubmit} class="p-6">
          <div class="space-y-4">
            <div>
              <label for="edit-entry-date" class="block text-sm font-medium text-gray-700">Date</label>
            ...
            <div>
              <label for="edit-entry-value" class="block text-sm font-medium text-gray-700">Value</label>
              <input 
                type="number" 
                id="edit-entry-value" 
                bind:value={value} 

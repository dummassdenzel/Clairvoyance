<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await api.createKpi({ name, target, rag_red, rag_amber, direction, format_prefix, format_suffix });
      ...
      <form on:submit|preventDefault={handleSubmit}>
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="kpi-direction" bind:value={direction} class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" id="kpi-format-prefix" bind:value={format_prefix} class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="e.g. $">
            </div>
            <div>
              <label for="kpi-format-suffix" class="block text-sm font-medium text-gray-700">Format Suffix</label>
              <input type="text" id="kpi-format-suffix" bind:value={format_suffix} class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="e.g. %">
            </div>
          </div>
        </div>
